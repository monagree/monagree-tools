...
function intro(opts) {
    if (opts && opts.version) {
        printCyan('Monagree CLI version 1.0.3, Last updated 21-11-23');
        return;
    }
    console.log(rainbowText('Welcome to the Monagree Develoeper CLI tool. Use the following commands for your awesome design (call all commands, except init, from your project root folder)'));
...
var commands = [
    new classes_1.cMeta('init', 'Initialize Monagree CLI in your project. use `monagree init -p` to print the config'),
...
function ncfg() {
    printErr('No config yet, run `monagree init` to create one');
}
function nfold() {
    printErr('Invalid. Call all commands (except init) from your project root folder');
}
function nsgn() {
...
function finInit1(loader, pid) {
    (0, helpers_1.addMonagreeSDK)(function (stg, msg) {
        ...
        if (stg == 1) {
            printOk('SDK added. Happy coding! Dont forget to use `monagree branch the-branch-id` to init all your pages');
        }
        else if (stg == 0) {
...
function proceedInit(ans) {
    ...
    catHelp('categories', 'Choose Category', function (c, e1) {
        var tc = { c1: c.c };
        ans = __assign(__assign({}, ans), tc);
        catHelp("categories/".concat(e1, "/cats"), 'Choose Sub-Category', function (c, e2) {
            var tc = { c2: c.c };
            ans = __assign(__assign({}, ans), tc);
            catHelp("categories/".concat(e1, "/cats/").concat(e2, "/subcat"), 'use "spaces" to select as many categories that apply', function (c) {
                ...
                (0, helpers_1.cdIntoProject)(pid, function (ok) {
                    if (ok) {
                        ...
                        var tc_1 = { c3: c.c };
                        ans = __assign(__assign(__assign({ id: (0, helpers_1.getCT)() }, ans), tc_1), { branches: { home: { branch_desc: homeDesc } } });
                        ...
                        (0, helpers_1.doBranch)('home', homeDesc, ans.fold, ans.lang, function (ok) {
                            if (ans.lang == 'ts') {
...
commander_1.program.command(commands[0].command)
    ...
    if (!(0, helpers_1.amInMonagreeFolder)()) {
        ...
        printErr('This folder contains an existing design. Please run init from a folder dedicated to all your designs');
    }
});
            ...
            (0, helpers_1.inquire)([
                ...
                if (ok) {
                    ...
                    if (cls_1.indexOf(a2.c) != -1) {
                        e = c1_1[cls_1.indexOf(a2.c)].getId();
                    }
                    finise(a2, e);
                }
                else {
...
commander_1.program.command(commands[1].command)
    ...
    var url = "https://developers.monagree.com/auth/".concat(maid, "/").concat((0, helpers_1.getDeviceToken)());
    printCyan(url);
    ...
    (0, helpers_1.inquire)([
        ...
        if (ok && ans.conf == 'yes') {
            ...
            fMan.getFS("mtks/".concat(maid + (0, helpers_1.getDeviceToken)()), function (task) {
...
commander_1.program.command(commands[3].command)
    ...
    (0, helpers_1.inquire)([
        ...
        if (ok) {
            (0, helpers_1.doBranch)(bid, dsc.branch_desc, cfg.fold, cfg.lang, function (ok) {
            ...
            if (opts.local) {
                ...
                (0, helpers_1.serveLocal)(function (ok) {
                    if (ok) {
                        ...
                        printCyan('Server is running at http://localhost:3310');
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    printNorm('- Until someone buys your design, it will be hosted in a repo on your github profile that is managed by Monagree');
                    ...
                    printNorm('- You may name the access token Monagree CLI\n');
        ...
        if (err || !stats || stats.hasErrors()) {
            ...
            if (Array.isArray(errors)) {
                errors.forEach(function (error, index) {
                    console.error("Error ".concat(index + 1, ": ").concat(JSON.stringify(error)));
                });
            }
            else {
            ...
            if (stats.hasWarnings()) {
                ...
                if (warnings && warnings.length > 0) {
                    ...
                    warnings.forEach(function (warning, index) {
                        console.warn("Warning ".concat(index + 1, ": ").concat(JSON.stringify(warning)));
                    });
                }
            }
            else {
